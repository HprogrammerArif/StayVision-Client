...
const ViewAllMaterials = () => {
  ...
  const {
    ...
    queryKey: ["material"],
    queryFn: async () => {
      const { data } = await axiosSecure(`/view-all-materials`);
      return data;
    },
  });
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosSecure.delete(`/materials/${id}`);
      console.log(data);
                ...
                <tbody className="bg-white divide-y divide-gray-200 ">
                  {material.map((job) => (
                    <tr key={job._id}>
                      ...
                      {/* Handle Delete And Handle Update Section */}

                      <td className="px-4 py-4 text-sm whitespace-nowrap">
                        <div className="flex items-center gap-x-6">
                          <button
                            onClick={() => handleDelete(job.materialId)}
                          ...
                          <Link
                            to={`updateMaterials/${job.materialId}`}
